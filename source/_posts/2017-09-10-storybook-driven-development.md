...
在讲这一部分之前，首先简单说明下，组件化开发方案是什么，组件化开发方案其实也就是UI模块还， 需要我们从架构层面对应用的UI进行模块化，一起把每一个UI页面抽象为一颗组件树，UI页面自己本身上也是一个组件。

## 前端在组件化开发遇到的困扰

那么我们组件化开发是不是就是一番风顺的呢，其实并不是的  我们也会遇到很多困扰
...
在大型的前端项目，经常遇到页面协同开发，于是这样的对话就产生了，这样就会尝试说 我们是否可以在页面没有ready的时候开始 我们组件的开发呢
